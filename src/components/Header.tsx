...
const Header = () => {
  ...
  return <>
      <header className="bg-gradient-to-r from-card/95 via-card to-card/95 backdrop-blur-xl border-b border-gold/20 sticky top-0 z-50 shadow-lg">
        <div className="container mx-auto px-4 py-3 flex-row flex items-center justify-between">
          <motion.div initial={{
          opacity: 0,
          x: -20
        }} animate={{
          opacity: 1,
          x: 0
        }} onClick={() => navigate("/")} className="flex items-center gap-2 cursor-pointer group">
          ...
          <div className="flex items-center gap-2">
            {user ? <>
              ...
              scale: 1
                  ...
                  <Button variant="ghost" size="icon" className="h-6 w-6 rounded-full bg-gold/20 hover:bg-gold/30 ml-1" onClick={() => setShowAddFunds(true)}>
                    <Plus className="w-3 h-3 text-gold" />
                  </Button>
                </motion.div>
                <DailyBonus />
                <Button variant="ghost" size="icon" onClick={() => navigate("/leaderboard")} className="hover:bg-gold/10">
                  <Trophy className="w-5 h-5 text-gold" />
                </Button>
                <Button variant="ghost" size="icon" onClick={() => navigate("/referral")} className="hover:bg-royal/10">
                  <Users className="w-5 h-5 text-royal" />
                </Button>
                {isAdmin && <Button variant="ghost" size="icon" onClick={() => navigate("/a9x7k2m4")} className="hover:bg-red-500/10" title="Admin Panel">
                ...
                <Button onClick={() => navigate("/auth")} className="bg-gradient-to-r from-gold to-gold-light text-navy-light font-bold hover:shadow-[var(--shadow-gold)]">
                  Sign In
                </Button>
              </>}
          </div>
      ...
      {user && <AddFundsModal open={showAddFunds} onClose={() => setShowAddFunds(false)} userId={user.id} onSuccess={() => fetchBalance(user.id)} />}
    </>;
};
