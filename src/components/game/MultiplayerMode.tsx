...
const MultiplayerMode = ({ userId, username, balance, onBack }: MultiplayerModeProps) => {
  ...
  const fetchCurrentRound = useCallback(async () => {
    ...
    const { data: newRoundId } = await supabase.rpc("get_current_round");
    if (newRoundId) {
  ...
  const fetchBets = useCallback(async () => {
    ...
    if (data) {
      ...
      if (myBet) {
        setUserBet({ side: myBet.side, amount: myBet.amount });
      } else {
        setUserBet(null);
      }
    }
  ...
  useEffect(() => {
    ...
    const betsChannel = supabase
      .channel("multiplayer-bets")
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "multiplayer_bets" },
        () => {
          fetchBets();
  ...
  const handlePlaceBet = async () => {
    ...
    try {
      const { data, error } = await supabase.rpc("place_multiplayer_bet", {
        p_round_id: currentRound.id,
        p_user_id: userId,
        p_username: username,
        p_side: selectedSide,
      ...
      setUserBet({ side: selectedSide, amount });
      setBetAmount("");
    ...
    <div className="min-h-screen bg-background p-4">
      ...
      <AnimatePresence>
        {(isFlipping || showResult) && (
          ...
          >
            ...
            >
              <img
                src={currentRound?.winner === "king" || (!showResult && !isFlipping) ? coinHeadsImage : coinTailsImage}
                alt="Coin"
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <motion.div
          ...
          className={`bg-gradient-to-br from-amber-900/30 to-amber-800/10 rounded-2xl p-4 border-2 transition-all ${
            selectedSide === "king"
              ? "border-primary shadow-[0_0_30px_hsl(var(--primary)/0.3)]"
              : "border-amber-700/30"
          } ${userBet?.side === "king" ? "ring-2 ring-primary" : ""}`}
          onClick={() => !userBet && setSelectedSide("king")}
        >
        ...
        <motion.div
          initial={{ opacity: 0, x: 50 }}
          animate={{ opacity: 1, x: 0 }}
          className={`bg-gradient-to-br from-slate-700/30 to-slate-600/10 rounded-2xl p-4 border-2 transition-all ${
            selectedSide === "tail"
              ? "border-secondary shadow-[0_0_30px_hsl(var(--secondary)/0.3)]"
              : "border-slate-600/30"
          } ${userBet?.side === "tail" ? "ring-2 ring-secondary" : ""}`}
          onClick={() => !userBet && setSelectedSide("tail")}
        >
          <div className="flex items-center gap-2 mb-3">
      ...
      {/* Bet Input */}
      {!userBet && currentRound?.status === "betting" && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <p className="text-center text-muted-foreground mb-3">
            {selectedSide
              ? `Betting on ${selectedSide === "king" ? "King Army" : "Tail Army"}`
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              disabled={!selectedSide || !betAmount || isPlacingBet}
